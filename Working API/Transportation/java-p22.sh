...
server_up() {
    echo "Server up..."
    docker pull 0taras0/p22-java:latest
    docker stop p22-java_container || true
    docker rm p22-java_container || true
    docker run -d --restart=always -v /volumes/p22-java/images:/app/uploads --name p22-java_container -p 6892:8345 0taras0/p22-java
}
